...
function YCCMemberList() {
    ...
    $memberQuery = "select 
                   ...
                   left join (
                     select xref.membershipNumber, Group_Concat(forum.real_name) as forumNames
                     from ycc_memberForumCR as xref
                     left join smf_members as forum
                     on xref.forumId = forum.id_member
                     group by xref.membershipNumber
                   ) as fNames
                   on members.membershipNumber = fNames.membershipNumber
