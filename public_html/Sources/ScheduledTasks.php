...
function scheduled_scnforumfirewall()
{
	global $boarddir, $sourcedir;
	require_once($sourcedir . '/Subs-ForumFirewallScan.php');

	$file_test = new ImageScanner($boarddir.'/Themes/default/images/');
	$file_test->run();

	$file_test = new ImageScanner($boarddir.'/avatars/');
	$file_test->run();

	$file_test = new ImageScanner($boarddir.'/Smileys/');
	$file_test->run();
	return true;
}
function scheduled_prgforumfirewall()
{
	global $ffcachedir, $boarddir;
	...
	if (empty($ftest)) return false;

	$type = $lastchanged = $cachedfile = '';
	$type = 'data'; //  This is prefix to the cache file
	$dh = opendir($ffcachedir);
	while (false !== ($file = readdir($dh))) {
		if ($file != '.' && $file != '..' && $file != 'index.php' && $file != '.htaccess' && (!$type || substr($file, 0, strlen($type)) == $type)) {
			$cachedfile = $ffcachedir . '/' . $file;
			$findme = 'ffirewall';
			$pos = strpos($cachedfile, $findme);
			if ($pos !== false) {  //  Identify a FF cache file
				$lastchanged = (file_exists($cachedfile) ? filemtime($cachedfile) : false);
				if (($lastchanged !== false) && (date('Y-m-d H:i:s', $lastchanged) <= date('Y-m-d H:i:s', strtotime('yesterday 23:59:59')))) {
					@unlink($cachedfile); }
	}	}	}
	closedir($dh);
	return true;
