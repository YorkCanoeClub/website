...
// ForumFirewall Start
$helptxt['forumfirewall_enable'] = 'Enables inspection of incoming traffic.  Each additional inspection must be checked to work.';
$helptxt['forumfirewall_enable_inj'] = 'Allows scanning all URI\'s for SQL injections.';
$helptxt['forumfirewall_enable_xxs'] = 'Reviews visitor\'s local site cookie content for control statements that are used in cross-site scripting (XXS) attacks.<br /><br />Scanning certain image files is controled in scheduled tasks menu.<br /><br />Cross-site scripting takes advantage of web application vulnerabilities that allow attackers to bypass client-side security mechanisms normally imposed on web content by modern browsers. There are many forms of attacks, the feature checks for bad cookies in the visitors computer.';
$helptxt['forumfirewall_enable_header'] = 'Protects against Enitity, Referrer and User-Agent Attacks in the HTTP Header.';
$helptxt['forumfirewall_enable_country'] = 'Identify visitors from bad countries.';
$helptxt['forumfirewall_enable_dos'] = 'This setting sets the watch time for fast and frequent reloading (F5 attack) and fast loading crawlers that adsorb bandwidth. The cache duration must be at least 20 seconds and the trigger will need to be set as high as desired for this feature to operate.';
$helptxt['forumfirewall_in_geoip'] = 'Select if using GeoIP, leave unchecked if using the http headers to identify the Country code. If checked, program will try to get the code from GeoIP. GeoIP only works with apache_note or $_SERVER("GEOIP_COUNTRY_CODE") where the DB is in the server.<br /><br />If this is not selected then, the program will search the headers for the Country Code as specified by you in "Country Code via Headers".<br /><br /><b>WARNING:</b>  GeoIP Apache API uses the REMOTE_ADDR ip and checks it against the GeoIP database.  This could cause a false negative or positive if the visitor uses a proxy.';
$helptxt['forumfirewall_enable_bypass'] = 'Confirms that visitors are not attempting to bypass your Proxy.';
$helptxt['forumfirewall_enable_check_ip'] = 'Review all incoming ipv4 & ipv6 addresses for proper structure.  This tests scans all the IPs in the visitors proxy list to prevent them from being used or combined in a DOS or resource starvation attack.<br />For example, if a visitor has a ip address of 0.0.0.0 the visitor will be flagged.';
$helptxt['forumfirewall_enable_proxy'] = 'Reviews all ip addresses available to visitor from the proxy list.<br /><br />If this feature is not selected the ip address the member is using will still be tested but the addresses that are available to the visitor through the proxy list will not be tested.  If you have a bad bot who is using this list to attack your site the bot can pull a ip address out and try to access with that address.  Only then will the address will be tested.<br /><br />If this feature is selected and a bad bot visits your site the entire proxy list will be tested and if a single ip is incorrect the bot can be blocked.  Saving you the hassle associated with the attack.<br /><br /><b>WARNING:</b> Enabling this feature may block mobile users. It has been reported that several major mobile phone proxies like the Kindle network have incorrect ips.';
$helptxt['forumfirewall_enable_block'] = 'Blocks all visitors who fail a test.<br /><br /><b>WARNING:</b>  It is recommended that you do not enable this feature until after you operated the mod for several days and you are fully confident that there are no infractions in the visitor logs that can deny you or your top members access.';
$helptxt['forumfirewall_logging'] = 'Logs all visitors who failed the specified test.';
$helptxt['forumfirewall_real_ip'] = 'Provide the real IP address of the visitor that is reported from the Proxy server through the HTTP environment (called through $_SERVER) . IE: CloudFlare is HTTP_CF_CONNECTING_IP.  Leave blank if you are not using a proxy.';
$helptxt['forumfirewall_header_id'] = 'Some proxy servers forward data packets with header lines such as HTTP_VIA, HTTP_X_FORWARDED_FOR, Cf-Connecting-Ip, X-Forwarded-For or HTTP_FORWARDED, that reveals that the Proxy is up and running.  Enter this identification header information here.  Without this information bypass protection and ip identification through the proxy will not work.';
$helptxt['forumfirewall_country_id'] = 'The feature is only active if GeoIP is not selected.  Enter the header array key to get the Country Code.  This is Cf-Ipcountry for ForumFirewall.';
$helptxt['forumfirewall_cache_duration'] = 'The buffer time in seconds that the SMF Caching System is allowed to keep information for a readable IP address of visitors. Because bots can look at multiple pages in your forum in a short amount of time this setting will block them throughout the duration of the buffer time. A good visitor will always have to pass the DOS SQL Injection test before leaving. Caching will use the least amount of processer effort.<br /><br />If the Caching System is not working you will get a warning message in the error log.<br /><br />The time delay can be set between 0 and 99 seconds. Zero disables this feature and prevents error log messages. This number should be adjusted high enough to adsorb the length of an average bot attack. Default is 20 seconds.';
$helptxt['forumfirewall_salt'] = 'A salt is for preventing rainbow table attacks against your cache.  If you salt your cache identifier string, you decrease the predictability of the hashed string, making it harder for an attacker to match anything in a rainbow table.<br /><br />Recommended length of the salt is 4-5 digits.  Longer salts are not necessary since the final string length is fixed to maintain forum speed.  In fact, if you make the salt too long it could possibly force all the cache identifier strings to be identical and render the cache useless.  The salt should be a combination of letters, numbers and printable special characters such as the ampersand (&).  During installation the mod attempts to automatically generate a 4 digit salt.';
$helptxt['forumfirewall_domain'] = 'Enter your site domain name here.';
$helptxt['forumfirewall_ip'] = 'Enter the IP address that the Proxy points to your domain name.';
$helptxt['forumfirewall_uri_chars'] = 'All characters not in this regular expression will fail the test.<br /><br />The regular expression will need to be adjusted if the language is not english.  This regular expression is similar to the popular CodeIgniter $config["permitted_uri_chars"] and a search will assist in the translation.<br /><br />The final test is performed in lower case.';
$helptxt['forumfirewall_exploits'] = 'If any visitor attacks that are typed in the browser are discovered the visitor will be flagged. This feature does not protect against information typed in a post by an approved member.  Codes must be entered in the format of "XX|YY" where XX and YY are the Attack Codes.';
$helptxt['forumfirewall_xxs'] = 'List of JavaScript Event Handlers, HTML and Scripting elements that do not belong in cookies. ActiveX, VBScript & Flash can be used in an attack but are not included in the initial list.';
$helptxt['forumfirewall_referer_attack'] = 'If any visitor from a different host than specified in the HTTP Header Referrer is discovered, the visitor\'s Referrer string will compared against the codes. Codes must be entered in the format of "XX|YY" where XX and YY are the Attack Codes.';
$helptxt['forumfirewall_ua_attack'] = 'If any visitor attacks that are in the HTTP Header User-Agent are discovered the visitor will be flagged. Codes must be entered in the format of "XX|YY" where XX and YY are the Attack Codes.';
$helptxt['forumfirewall_entity_attack'] = 'If Request Enity exists it should be checked for code injection through the request entity header.  Most of the time this value does not exist but, when it does it will be inspected against the this list.<br /><br />Request Entities must be entered in the format of "XX|YY" where XX and YY are the Entity.';
$helptxt['forumfirewall_bad_countries'] = 'Country Codes must be entered in the format of "XX|YY" where XX and YY is the Country Code.';
$helptxt['forumfirewall_enable_ua'] = 'Identifies an attempt to mask the User-Agent that is used in some DOS Attacks.';
$helptxt['forumfirewall_enable_email'] = 'Select wether you want a mail sent to the administrator account on Blocked or Logging events';
$helptxt['forumfirewall_trigger'] = 'Number of violations per second during the cache period before the visitor is sent flagged.  Minimum is ".4" (no bots), default is ".65" (assuming bots).';
$helptxt['forumfirewall_longterm_ban'] = 'Use SMFs banning feature to ban based on multiple infractions or connection tempts as specified in "Trigger".  Select the time period the violators are set to be banned.<br /><br />This feature will not work if cache is less than 20.  Default is 24 Hours.';
$helptxt['forumfirewall_enable_admin'] ='Confirms that the admin\'s ip address is between the high and low range.  This feature only works with ipv4 addresses.  It is used to prevent admin cookie hacking.<br /><br />If using a proxy and the proxy fails this feature could block you from accessing the site.  This will be prevented if you complete the "Proxy Header ID" field.<br /><br />This feature is <b>DANGEROUS</b>.  If your ip address changes the only way you can get access to your forum is by editing the database in phpmyadmin.';
$helptxt['forumfirewall_admin_ip_lo'] ='The low ip Range for the admin.  Must be a valid ipv4 address and be in the form of "xx.xx.xx.xx".';
$helptxt['forumfirewall_admin_ip_hi'] ='The high ip Range for the admin.  Must be a valid ipv4 address and be in the form of "xx.xx.xx.xx".  If you have one ip address then make this the same as "Admin ip Low".';
$helptxt['forumfirewall_admin_domain'] ='Enter the admin domain name here to prevent ip spoofing of the admin\'s ip address. This feature only works with ipv4 addresses.<br /><br />The domain name should be shortened.  For Example if your address is "234-249-67-50.homeaddress.com" enter "homeaddress.com" as the admin domain name.<br /><br />If this cell is left blank the anti-spoofing test will not be conducted.';
$helptxt['forumfirewall_enable_robots'] = 'The test is effective at preventing some forms of ddos attacks.  The mod validates the action array calls of the visiting robots and blocks them if they fail to follow the robots.txt instructions.';
$helptxt['forumfirewall_test_robots'] = 'List the User Agents of the bots to be tested.  Major search engines that follow robots.txt should be tested.<br /><br />User Agents must be entered in the format of "XX|YY" where XX and YY are the Entity.';
$helptxt['forumfirewall_robotstxt_action'] = 'When installed the mod attempts to read the action items in your sites Robots.txt file.  If there are not action items, it is recommended that you edit your robots.txt file.  The format of the robots.txt should be as follows:<br /><br />User-agent: *<br /><br />Disallow: /smf/index.php?action=activate<br />Disallow: /smf/index.php?action=admin<br /><br />Action array values must be entered in the format of "XX|YY" where XX and YY are the Entity. ie "action=activate|action=admin".';
$helptxt['forumfirewall_enable_rmtport'] = 'Unix, Linux, Mac and some other servers may look at ports less than 1024 as Privileged. Based on your server you may want to choose this option to block access to all ports less than 1024 and greater than 65535.<br /><br />This feature may block valid users who use proxies.';
$helptxt['forumfirewall_enable_svrport'] = 'Some versions of Apache are subject to a memory-corruption vulnerability.  Restricting access to your forum through certain server ports may prevent possible hacking attempts.';
$helptxt['forumfirewall_good_ser_ports'] = 'The Server Ports in this list will be allowed to access your forum.  All other server ports will be blocked.  Ports must be entered in the format of "XX|YY" where XX and YY are the ports.';
// ForumFirewall End

// ForumFirewall 2x Start
$helptxt['forumfirewall_good_ua'] = 'The User-Agents in list will not be subject to a DOS test.  User-Agents must be entered in the format of "XX|YY" where XX and YY are the User-Agents.<br /><br />You can control the good robots that visit your site by creating a "robots.txt" file and placing it in your root directory. People usually give them a 5 or 10 second delay. Most follow this file, except for Google. You can control Google at: "http://www.google.com/support/webmasters/".<br /><br />Individual member groups are selected in Admin/Members/Manage Permissions: Forum Firewall Whitelist Group.';
// ForumFirewall 2x End
