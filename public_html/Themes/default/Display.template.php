		...
		if (!$message['member']['is_guest'])
		{
			...
			if ($message['member']['can_see_warning'])
				echo '
                        ...
                        $memberQuery = "select 
                                        concat(members.membershipNumber, concat('/', members.membershipType)) as memberships,
                                        membershipStatus as status
                                       from ycc_members as members 
                                       join ycc_memberForumCR as crossref
                                       on members.membershipNumber = crossref.membershipNumber
                                       where crossref.forumId = '" . $message['member']['id'] .
                                       "' and crossref.primaryMember = 1
                        ...
                        if (mysql_num_rows($result) == 1 && $row = mysql_fetch_assoc($result)) {
                            ...
                            echo '<li>&nbsp;</li>';
                            echo '<li><strong>YCC No.:</strong> ', $row['memberships'], '</li>';
                            echo '<li><strong>YCC Status:</strong> ', getStatusDesc($row['status']), '</li>';
